...
# Stage-1 gate for the versioned cooperative Intent-v1 environment.
# Train only Local, Type-Oracle and Last-action for one seed. Do not start a
# Belief-v2 experiment until these baselines demonstrate useful headroom.
project_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$project_root" || exit 1

output_dir="${OUTPUT_DIR:-results/intent_v1_headroom}"
...
echo "All Intent-v1 headroom trainings completed."
